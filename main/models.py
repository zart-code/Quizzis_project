...
def log_quiz_save(sender, instance, created, **kwargs):
    ...
    if created:
        logger.info(f"[DB CREATE] Квиз: '{instance.title}' (ID: {instance.id}) создан пользователем {instance.creator.username}")
    else:
...
def log_gamesession_save(sender, instance, created, **kwargs):
    ...
    if created:
        logger.info(f"[GAME LOBBY] Создана сессия PIN: {instance.pin} для квиза '{instance.quiz.title}' (хост: {instance.host.username})")
...
def log_participant_join(sender, instance, created, **kwargs):
    ...
    if created:
        logger.info(f"[GAME JOIN] Игрок {instance.user.username} присоединился к сессии {instance.session.pin}")
...
def log_quiz_result(sender, instance, created, **kwargs):
    ...
    if not created and instance.completed:
        logger.info(f"[QUIZ COMPLETE] Пользователь {instance.user.username} завершил квиз '{instance.quiz.title}': {instance.score}/{instance.max_score} ({instance.score_percent:.1f}%)")