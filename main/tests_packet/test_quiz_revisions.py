...
from main.services.quiz_revisions import (
    get_current_revision, get_revision_questions, get_quiz_questions,
    build_revision_payload, collect_question_payloads_from_post,
    calculate_revision_totals, create_revision_from_payloads,
    build_quiz_form_payload, build_quiz_payload_for_edit
)
...
class TestRevisions(TestCase):
    ...
    def test_get_quiz_questions_with_revision(self):
        question_payload = {
            'text': 'dasda', 'question_type': 'multiple', 'time_limit': 30,
            'coefficient': 1, 'order': 1, 'correct_number': None,
            'answers': [
    ...
    def test_build_revision_payload(self):
        question_payload = {
            'text': 'Revision question?', 'question_type': 'single', 'time_limit': 30,
            'coefficient': 1, 'order': 1, 'correct_number': None,
            'answers': [
    ...
    def test_create_revision_from_payloads(self):
        payloads = [{
            'text': 'Q', 'question_type': 'single', 'time_limit': 30,
            'coefficient': 2, 'order': 1, 'correct_number': None,
            'answers': [
    ...
    def test_build_quiz_form_payload(self):
        question_payloads = [{'text': 'Q', 'question_type': 'single', 'time_limit': 20,
                              'coefficient': 1, 'correct_number': None,
