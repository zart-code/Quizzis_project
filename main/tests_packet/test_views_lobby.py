...
class LobbyViewsTest(TestCase):
    ...
    def test_create_lobby_for_draft_quiz(self):
        draft_quiz = Quiz.objects.create(title='Draft', creator=self.teacher, status=Quiz.DRAFT)
        self.client.force_login(self.teacher)
    ...
    def test_join_lobby_view_success(self):
        ...
        self.assertTrue(GameParticipant.objects.filter(session=self.session, user=self.student).exists())
    ...
    def test_join_lobby_view_when_already_joined(self):
        ...
        response = self.client.get(reverse('join_lobby', args=[self.session.pin]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(GameParticipant.objects.filter(session=self.session, user=self.student).count(), 1)
    ...
    def test_session_play_view_get_in_progress(self):
        self.client.force_login(self.student)
        participant = GameParticipant.objects.create(session=self.session, user=self.student)
        self.session.status = 'in_progress'
    ...
    def test_session_play_view_post_answer_correct(self):
        ...
        response = self.client.post(reverse('session_play', args=[self.session.pin]), data)
    ...
    def test_session_results_teacher_view(self):
        self.client.force_login(self.teacher)
        participant = GameParticipant.objects.create(session=self.session, user=self.student, score=4)
        question = self.quiz.questions.first()
        GameAnswer.objects.create(
            session=self.session, participant=participant, question=question,
            is_correct=True, points=4
        )
        response = self.client.get(reverse('session_results_teacher', args=[self.session.pin]))
        self.assertEqual(response.status_code, 200)
