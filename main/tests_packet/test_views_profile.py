...
class ProfileViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        QuizResult.objects.get_or_create(
            user=self.user1, quiz=self.quiz,
            defaults={'score': 5, 'max_score': 10, 'score_percent': 50.0, 'completed': True}
        )
        QuizResult.objects.get_or_create(
            user=self.user2, quiz=self.quiz,
            defaults={'score': 9, 'max_score': 10, 'score_percent': 90.0, 'completed': True}
        )
    ...
    def test_profile_view_other_as_admin(self):
        ...
        response = self.client.get(reverse('admin_user_profile', kwargs={'user_id': self.user1.id}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['profile_user'], self.user1)
    ...
    def test_profile_view_other_as_non_admin(self):
        self.client.force_login(self.user1)
        response = self.client.get(reverse('admin_user_profile', kwargs={'user_id': self.user2.id}))
    ...
    def test_profile_history_view_other_as_admin(self):
        self.client.force_login(self.user2)
        response = self.client.get(reverse('admin_user_history', kwargs={'user_id': self.user1.id}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['profile_user'], self.user1)
        self.assertTrue(response.context['is_admin_view'])
    ...
    def test_profile_history_view_other_as_non_admin(self):
        self.client.force_login(self.user1)
        response = self.client.get(reverse('admin_user_history', kwargs={'user_id': self.user2.id}))
        self.assertRedirects(response, reverse('profile'))
    ...
    def test_edit_profile_own_post_change_password(self):
        ...
        self.assertTrue(self.client.login(username=self.user1.username, password='newpassword123'))
    ...
    def test_edit_profile_other_as_admin(self):
        ...
        response = self.client.post(reverse('admin_edit_user', kwargs={'user_id': self.user1.id}), data)
        self.assertRedirects(response, reverse('admin_user_profile', kwargs={'user_id': self.user1.id}))
        self.user1.refresh_from_db()
    ...
    def test_edit_profile_other_as_non_admin(self):
        self.client.force_login(self.user1)
        response = self.client.get(reverse('admin_edit_user', kwargs={'user_id': self.user2.id}))
        self.assertRedirects(response, reverse('profile'))
    ...
    def test_edit_profile_admin_edit_without_password(self):
        ...
        response = self.client.post(reverse('admin_edit_user', kwargs={'user_id': self.user1.id}), data)
        self.assertRedirects(response, reverse('admin_edit_user', kwargs={'user_id': self.user1.id}))
        messages = list(response.wsgi_request._messages)
