...
class ProfileViewsTest(TestCase):
    ...
    def test_profile_view_other_as_admin(self):
        ...
        response = self.client.get(reverse('admin_user_profile', args=[self.student.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context['is_admin_view'])

    def test_profile_view_other_as_regular(self):
        self.client.force_login(self.student)
        response = self.client.get(reverse('admin_user_profile', args=[self.teacher.id]))
    ...
    def test_edit_profile_own_post(self):
        self.client.force_login(self.student)
        data = {'username': 'newname', 'email': 'new@mail.com', 'password': '', 'password_confirm': ''}
    ...
    def test_edit_profile_own_change_password(self):
        ...
        response = self.client.post(reverse('edit_profile'), data)
        self.assertRedirects(response, reverse('profile'))
        self.assertTrue(self.client.login(username=self.student.username, password='newpass123'))
    ...
    def test_edit_profile_admin_edits_other(self):
        ...
        response = self.client.post(reverse('admin_edit_user', args=[self.student.id]), data)
        self.assertRedirects(response, reverse('admin_user_profile', args=[self.student.id]))
        self.student.refresh_from_db()
    ...
    def test_profile_history_view_other_as_admin(self):
        self.client.force_login(self.admin)
        response = self.client.get(reverse('admin_user_history', args=[self.student.id]))
        self.assertEqual(response.status_code, 200)