...
from apps.quiz.models import Quiz, QuizRevision, Question, Answer, QuizReport
...
class TestRevisions(TestCase):
    ...
    def setUpTestData(cls):
        """Создание тестового квиза и связанных объектов."""
        cls.author = User.objects.create_user(username='author', password='testpass')
        cls.quiz = Quiz.objects.create(
            title='dsadda',
            creator=cls.author,
            status='published'  # или другой актуальный статус
        )
        # Создаём один вопрос для тестов, где нужен хотя бы один вопрос
    ...
    def test_get_current_revision_exists(self):
        ...
        rev = QuizRevision.objects.create(
            quiz=self.quiz, version=1, title='v1'
        )
...
class ScoringStrategiesTest(TestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        """Создание общего квиза и автора."""
        cls.author = User.objects.create_user(username='scoring_author', password='testpass')
        cls.quiz = Quiz.objects.create(
            title='Scoring Quiz',
            creator=cls.author,
            status='published'
