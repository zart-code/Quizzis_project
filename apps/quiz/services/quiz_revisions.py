...
def get_revision_questions(revision):
    ...
    return list(revision.questions.prefetch_related("answers").order_by("order", "id"))
...
def get_session_questions(session):
    ...
    if session.revision_id:
        return list(
            session.revision.questions.prefetch_related("answers").order_by(
                "order", "id"
            )
        )
...
def build_quiz_payload_for_edit(quiz):
    ...
    return {
        ...
        "questions": [
            ...
            for question in quiz.questions.prefetch_related("answers").order_by(
                "order", "id"
            )
        ],
    }
